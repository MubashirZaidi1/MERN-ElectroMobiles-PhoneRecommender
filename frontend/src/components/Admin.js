...
import './css/Admin.css'; // Import your CSS file for styling
...
const Admin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isSignUp) {
        ...
        const response = await axios.post('/api/admins/signup', { email, password });
  ...
  const handleSignUpToggle = () => {
    setIsSignUp(!isSignUp); // Toggle between login and sign-up mode
  };
  ...
  if (authenticated) {
    ...
    ); // Render Dashboard component if authenticated
  }

  return (
    <form className="form" onSubmit={handleSubmit}>
      <div className="title">Welcome,<br /><span>{isSignUp ? 'sign up to continue' : 'log in to continue'}</span></div>
      <input
        className="input"
        name="email"
        placeholder="Email"
      ...
      <button type="button" className="toggle-btn" onClick={handleSignUpToggle}>
